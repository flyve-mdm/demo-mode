 ...
 * @since 0.1.0
 */
class PluginFlyvemdmdemoUser extends User
{

   static $rightname = 'flyvemdmdemo:user';
   ...
   public static function getTypeName($nb=0) {
      return _n('FlyveMDM user', 'FlyveMDM users', $nb, 'flyvemdmdemo');
   }
   ...
   public static function canCreate() {
      $config = Config::getConfigurationValues('flyvemdmdemo', array('registered_profiles_id'));
   ...
   public function prepareInputForAdd($input) {
      ...
      if (!$input['name']) {
         ...
         Session::addMessageAfterRedirect(__('Email address is not valid', 'flyvemdmdemo'));
         return false;
      }
      ...
      self::$creation = false;
      if ($entityId === false) {
         Session::addMessageAfterRedirect(__('An entity already exists for your email. You probably already have an account.', 'flyvemdmdemo'));
         return false;
      }
      ...
      $fleetId = $this->defaultFleet->getFromDBByDefaultForEntity($entityId);
      if ($fleetId === false) {
         $this->defaultFleet = null;
         $this->entity->delete(['id' => $entityId]);
         $this->entity = null;
         Session::addMessageAfterRedirect(__('Failed to create default fleet', 'flyvemdmdemo'));
         return false;
      }
   ...
   protected function checkPassword($input) {
      if ($input['password'] != $input['password2']) {
         ...
         Session::addMessageAfterRedirect(__('Passwords are different', 'flyvemdmdemo'));
         return false;
      }
         ...
         if ($profile) {
            ...
            if ($demoMode) {
               ...
               NotificationEvent::raiseEvent(
                   PluginFlyvemdmdemoNotificationTargetAccountvalidation::EVENT_SELF_REGISTRATION,
                   $accountValidation,
